...
const PostItem = ({
  ...
  const hasLiked = session
    ? displayPost.likes.includes(session.user.id)
    : false;
  const hasReposted = session
    ? displayPost.repostedBy.includes(session.user.id)
    : false;
  ...
  return (
    <div
      className={clsx("relative flex gap-3 p-4", {
    ...
    >
      ...
      <div className="flex-1">
        ...
        <div className="flex items-center gap-2">
          <Link
            href={`/profile/${displayPost.author.name}`}
            className="font-bold hover:underline"
          >
            {displayPost.author?.name || "Unknown User"}
          </Link>
          <p className="text-sm text-neutral-500">
            @{displayPost.author?.email?.split("@")[0] || "unknown"}
          </p>
          ...
          <div className="mt-2">
            ...
            <div className="mt-2 flex justify-end gap-2">
              <button
                ...
                className="px-4 py-1.5 text-sm font-bold rounded-full hover:bg-neutral-800"
              >
                Cancel
              </button>
              <button
                onClick={handleSaveEdit}
                className="px-4 py-1.5 text-sm font-bold rounded-full bg-white text-black"
              >
                Save
              </button>
            </div>
          ...
          <div className="mt-3 flex items-center justify-between text-neutral-500 max-w-xs">
            <CommentModal post={displayPost}>
              <button className="group flex items-center gap-2">
                <MessageCircle
                  size={18}
                  className="transition-colors group-hover:text-sky-500"
                />
      ...
      {!isEditing && !isEmbedded && (
        <PostActionsMenu post={post} onEdit={() => setIsEditing(true)} />
      )}
    </div>
  );
};
